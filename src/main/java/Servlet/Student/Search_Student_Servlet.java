...
public class Search_Student_Servlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (loginSession == null || loginSession.getAttribute("username") == null) {
            ...
            List<Student> listsv = st.findName(name);
            if (!listsv.isEmpty()) {
                req.setAttribute("listsv", listsv);
                RequestDispatcher rd = getServletContext().getRequestDispatcher("/Student/student_search.jsp");
                rd.forward(req, resp);
            } else {
                Object searchError = "Student not found!";
